...
{
    public class DummyLegoCollectionsDocumentClient : IDocumentClient
    {
        ...
        public Task<FeedResponse<dynamic>> ReadDocumentFeedAsync(Uri documentCollectionUri, FeedOptions options = null)
        {
            ...
            foreach (var legoCollection in _legoCollections)
            {
                var newLegoCollection = new LegoCollection()
                {
                    Id = legoCollection.Id
                };
                newLegoCollection.LegoSets.AddRange(legoCollection.LegoSets);
                newLegoCollection.LegoBlocks.AddRange(legoCollection.LegoBlocks.Select(l => new LegoBlockCollected()
                 {
                     Id = l.Id,
                     Amount = l.Amount
                 }));
                docs.Add(newLegoCollection);
            }
